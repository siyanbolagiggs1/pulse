...
interface CampaignPreviewCardProps {
  ...
  /** Fetch and show the target URL's Open Graph image/domain. Default true. */
  showPreview?: boolean;
}

export function CampaignPreviewCard({ campaign, href, footer, showPreview = true }: CampaignPreviewCardProps) {
  const [preview, setPreview] = useState<LinkPreview | null>(null);
  const [loading, setLoading] = useState(showPreview);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-1 p-3">
        ...
        {showPreview && (
          <p className="truncate text-xs text-muted-foreground">{preview?.domain ?? " "}</p>
        )}
